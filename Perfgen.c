...
--*/
{
    ...
    for (dwThisInstance = 0; dwThisInstance < NUM_INSTANCES; dwThisInstance ++) {
		...
		{
			CONSTANT_VALUE_VALUE = dwInputValue;
		}
        ...
        // save sine value
        pSC->dwSineWaveValue = (DWORD) lValue; // Замена value на мои из регистров или через pipe
        ...
        // save triangle value
        pSC->dwTriangleWaveValue = (DWORD) lValue; // И ТУТА 
        ...
        pSC->dwConstantValue = CONSTANT_VALUE_VALUE;
